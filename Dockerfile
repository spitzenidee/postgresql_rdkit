...
RUN cmake -DRDK_BUILD_INCHI_SUPPORT=ON -DRDK_BUILD_PGSQL=ON -DRDK_BUILD_AVALON_SUPPORT=ON -DPostgreSQL_TYPE_INCLUDE_DIR="/usr/include/postgresql/9.6/server" -DPostgreSQL_ROOT="/usr/lib/postgresql/9.6" .. && make -j 3 && make install && sh Code/PgSQL/rdkit/pgsql_install.sh
